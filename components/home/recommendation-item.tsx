...
export const RecommendationItem = ({ data }: { data: any }) => {
  return (
    <div
      className={`rounded-[4px] md:rounded-[4.9px] flex overflow-hidden max-sm:w-[160px] relative ${
        data.mobile && "max-sm:hidden"
      }`}
      style={{ backgroundColor: data.bg, color: data.color }}
      // hidden={data.mobile}
    >
      <div className="space-y-6  pt-5 pb-2.5 px-2 md:px-2.5">
        <h2 className="font-bold leading-none tracking-[-1.572px] sm:text-sm md:text-[19.65px]/[19px] max-sm:min-w-[80px]">
          {data.title}
        </h2>
        <div>
          <Link
